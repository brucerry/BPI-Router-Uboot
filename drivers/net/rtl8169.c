...
/* Vendor warm reset / register cleanup for RTL8125 devices.
 * Soft MAC reset + clear preset/force registers + clear RxDv_Gated_En.
 */
...
/*
 * Try a device-level reset via U-Boot's DM PCI reset helper, if available.
 * Returns 0 on success, negative on failure or if helper not available.
 */
static int rtl_try_dm_pci_reset(struct udevice *dev)
{
#if defined(CONFIG_DM_PCI)
	/* Many U-Boot versions provide dm_pci_reset() or pci_reset_device().
	 * Try a couple of possibilities guarded by weak-looking ifdefs.
	 */
	#ifdef CONFIG_DM_PCI_RESET
		/* symbolic guard if your tree defines this */
		if (dm_pci_reset)
			return dm_pci_reset(dev);
	#endif

	/* Some U-Boot builds expose pci_reset_device() */
	#ifdef CONFIG_PCI_RESET
		if (pci_reset_device)
			return pci_reset_device(dev);
	#endif

	/* Try common name - dm_pci_reset_device */
	#ifdef CONFIG_DM_PCI_RESET_DEVICE
		if (dm_pci_reset_device)
			return dm_pci_reset_device(dev);
	#endif

	/* If we don't know a named helper, try calling pci_dev_reset if present */
	#ifdef CONFIG_PCI
		/* Some builds expose pci_dev_reset(dev) */
		extern int pci_dev_reset(struct udevice *bus);
		/* Best-effort call; may not link on some trees */
		return pci_dev_reset(dev);
	#else
		return -ENOSYS;
	#endif
#else
...
/*
 * Aggressive PCI config-space reset fallback:
 * - Save PCI command register
 * - Clear command (disables MEM and BUSMASTER)
 * - Try to find PCI PM capability and force D0 (PMCSR = 0)
 * - Restore command
 */
static void rtl_try_pci_reset_aggressive(struct udevice *dev)
{
	...
	/* Read and save command (offset 0x04, 32-bit read covers it) */
	...
	if (ret) {
		printf("rtl: cannot read capability pointer: %d\n", ret);
		cap_ptr = 0;
	}
	...
	while (cap_ptr) {
		...
		if (ret) {
			printf("rtl: capability read failed at 0x%02x (%d)\n", cap_ptr, ret);
			break;
		}
		if (cap_id == 0x01) { /* Power Management */
			ret = dm_pci_read_config16(dev, cap_ptr + 4, &pmcsr);
			if (ret == 0) {
				printf("rtl: PMCSR before 0x%04x\n", pmcsr);
				/* Force D0 (PMCSR bits [1:0] = 00) */
				dm_pci_write_config16(dev, cap_ptr + 4, 0x0000);
				udelay(500);
				printf("rtl: PMCSR forced to D0\n");
			}
			break;
		}
		/* next capability pointer at cap_ptr + 1 */
...
static int rtl8169_eth_probe(struct udevice *dev)
{
	...
	/* If this is RTL8125, try a device-level reset (FLR) first if available.
	 * If not available or it fails, try an aggressive config-space reset,
	 * then run vendor warm-reset/cleanup so MMIO writes take effect.
	 */
	if (pplat->device == 0x8125) {
		int rc = rtl_try_dm_pci_reset(dev);
		if (rc == 0) {
			printf("rtl: dm PCI reset/FLR succeeded\n");
		} else {
			printf("rtl: dm PCI reset/FLR not available or failed (%d), doing aggressive fallback\n", rc);
